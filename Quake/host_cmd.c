...
char* GetLastSavedFile(char* output, size_t output_size) {
	...
	int i;
	char latest_filename[16] = "";  // "s19.sav" + null terminator

	char save_dir[MAX_PATH];
	GetSaveDir(save_dir, NULL);

	if (!save_dir || !output || output_size == 0) {
		return NULL; // Invalid arguments
	}

	output[0] = '\0'; // Initialize output to empty string

	// Iterate through possible save files (s0.sav to s19.sav)
	for (i = 0; i < 20; i++) {
		// Filename only
		char filename[16];
...
#endif
		{
			...
			if (st.st_mtime > latest_time) {
				...
				latest_filename[sizeof(latest_filename) - 1] = '\0'; // Ensure null-termination
			}
		}
	}

	// If any file was found, copy filename to output
