...
char* to_utf8(const char* str) {
	...
	// Get a buffer for the UTF-8 output
	char* utf8_buf = get_utf8_buffer();

	// Temporary buffer for the intermediate wide string
	wchar_t wide_str[1024]; // Fixed size, sufficient for typical file paths

	// Step 1: Determine the size needed for the wide string and convert
	int wide_len = MultiByteToWideChar(CP_ACP, 0, str, -1, NULL, 0);
	if (wide_len == 0 || wide_len > 1024) {
		// Conversion failed or buffer too small; return empty string
		utf8_buf[0] = '\0';
		return utf8_buf;
	}
	MultiByteToWideChar(CP_ACP, 0, str, -1, wide_str, wide_len);

	// Step 2: Determine the size needed for UTF-8 and convert
